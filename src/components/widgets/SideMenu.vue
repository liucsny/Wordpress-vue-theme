<template>
  <div class="menu-container" @mouseover="changeOpenMenu(null)">
    <ul v-for="(categories, key) in list" :key="key" class="menu-ul">
      <div class="menu-category-name" @mouseover.stop="changeOpenMenu(categories.name)" @mouseout="mouseOut">{{categories.name}}</div>
      <!-- <div :class="{'menu-open': categories.name == openMenu, 'menu-close': categories.name !== openMenu}"> -->
      <div @mouseover.stop="changeOpenMenu(categories.name)" :ref="categories.name+'_subMenu'" class="sub-menu">
